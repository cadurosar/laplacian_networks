...
def main():
        ...
        for batch_idx, (inputs, targets) in enumerate(trainloader):
            ...
            if uses_regularizer:
                ...
                loss2 = _lambda * gsp.force_smooth_network(
                    relus, targets, m=args.m, classes=100)
                loss = loss + loss2
    ...
    epoch_start = [0,  60,  120, 160]
    epoch_end = [60, 120, 160, 200]
    for period in range(4):
        if period == 0:
            ...
            optimizer = torch.optim.SGD(
                net.parameters(), lr=0.02, momentum=0.9, weight_decay=5e-4)
        elif period == 2:
            optimizer = torch.optim.SGD(
                net.parameters(), lr=0.004, momentum=0.9, weight_decay=5e-4)
        else:
            optimizer = torch.optim.SGD(
                net.parameters(), lr=0.0008, momentum=0.9, weight_decay=5e-4)
